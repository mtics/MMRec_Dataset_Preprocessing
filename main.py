...
def quick_start(config, dataset_name):
    ...
    dataset_config = read_yaml_to_dict(
        os.path.join(dataset_base, dataset_folder, "config.yaml")
    )
    ...
    user_id, item_id, rating, timestamp, min_u_num, min_i_num, ratios = (
        dataset_config["field"]["user_id"],
        dataset_config["field"]["item_id"],
        dataset_config["field"]["rating"],
        dataset_config["field"]["timestamp"],
        config["config"]["min_user_interactions"],
        config["config"]["min_item_interactions"],
